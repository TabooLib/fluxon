...
class FunctionSystemTest {
    ...
    void printAndErrorCanTargetCustomStreams() {
        PrintStream previousOut = FunctionSystem.getOut();
        PrintStream previousErr = FunctionSystem.getErr();
        ...
        try {
            FunctionSystem.setOut(customOut);
            FunctionSystem.setErr(customErr);

            Fluxon.eval("print(\"hello\")");
            Fluxon.eval("error(\"oops\")");
            ...
            FunctionSystem.setOut(previousOut);
            FunctionSystem.setErr(previousErr);
            customOut.close();
            customErr.close();
        }
    }
