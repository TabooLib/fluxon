...
public class FluxonRuntimeTest {
    ...
    public static void registerTestFunctions() {
        ...
        // checkGrade 函数
        runtime.registerFunction("checkGrade", 1, (target, args) -> {
        ...
        // player 函数 - 支持多种参数数量
        runtime.registerFunction("player", Arrays.asList(1, 3), (target, args) -> {
            if (args.length >= 1) {
        ...
        // fetch 函数
        runtime.registerFunction("fetch", 1, (target, args) -> {
            if (args.length > 0) {
