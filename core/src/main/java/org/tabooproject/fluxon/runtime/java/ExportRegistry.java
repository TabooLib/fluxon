...
public class ExportRegistry {
    ...
    public ClassBridge registerClass(Class<?> clazz) {
        ...
        return registerClass(clazz, exportMethods);
    }
     ...
     * @return ClassBridge
     */
    public ClassBridge registerClass(Class<?> clazz, Method[] exportMethods) {
        // 为整个类生成或获取桥接器
        ClassBridge bridge = ExportBytecodeGenerator.generateClassBridge(clazz, exportMethods, clazz.getClassLoader());
        // 注册该类的所有导出方法
        registerClassMethods(clazz, exportMethods, bridge);
        return bridge;
    }
