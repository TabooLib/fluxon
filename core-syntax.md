...
通过命名约定识别常量：全大写标识符（模式 `[A-Z][A-Z0-9_]*`）赋值字面量时自动成为常量。
...
常量特性：
- 不可重新赋值：`PI = 6.28` 会报错 `Cannot reassign constant: PI`
- 编译时内联：`&PI` 在解析阶段被替换为字面量值，无运行时开销

## 字面量与集合

- 数字：`<int>`、`<long>L`、`<float>f`、`<double>`、科学计数法如 `2.5e3`
- 字符串：`"..."` 或 `'...'`（支持常见转义）
- 字符串插值：`"Hello ${<expr>}!"`（表达式结果自动转为字符串）
...
在字符串中使用 `${<expr>}` 嵌入表达式，表达式结果会自动转为字符串拼接：
